...
Proof.
  ...
  - apply T_Cons. apply IHHstep. assumption. assumption.
  - apply T_Cons. assumption. apply IHHstep. assumption.
  - apply T_Head with (n := n). apply IHHstep. assumption.
  - apply T_Tail. apply IHHstep. assumption.
  - match goal with H : has_type (tcons _ _) _ |- _ => inversion H; subst end.
    assumption.
  - match goal with H : has_type (tcons _ _) _ |- _ => inversion H; subst end.
    assumption.
Qed.
...
Proof.
  ...
  - left. constructor.
  - left. constructor.
  - destruct IHHt1 as [Hvhd | [hd' Hshd]].
    + destruct IHHt2 as [Hvtl | [tl' Hstl]].
  ...
  - right. destruct IHHt as [Hv | [v' Hs]].
    + inversion Hv; subst;
        try solve [match goal with
                   | Hx : has_type _ (TVec (S _)) |- _ => inversion Hx
                   end].
      exists hd. apply S_HeadCons; assumption.
    + exists (thead v'). apply S_Head. assumption.
  - right. destruct IHHt as [Hv | [v' Hs]].
    + inversion Hv; subst;
        try solve [match goal with
                   | Hx : has_type _ (TVec (S _)) |- _ => inversion Hx
    ...
    + exists (ttail v'). apply S_Tail. assumption.
Qed.
